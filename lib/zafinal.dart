...
import 'package:my_flutter_app/Surahlist.dart';
...
import 'MuhammadDb_helper.dart';
import 'quran_combined.dart';

void main() async {
  WidgetsFlutterBinding.ensureInitialized();
  await Hive.initFlutter();
  await Hive.openBox('allah_names');
  await DBHelper().initializeDatabase();
  await Hive.openBox('prophet_names');
  await MDBHelper().initializeDatabase();
  try {
...
class _HomeScreen1State extends State<homeScreen1> {
  ...
  Future<void> fetchRandomProphetName() async {
    ...
    if (box.isNotEmpty) {
      ...
      setState(() {
        prophetName = randomProphet["Arabic"];
        prophetTransliteration = randomProphet["Name"];
    ...
    return Row(
      ...
      children: [
        ...
        _buildFeatureButton(
          ...
          onTap: () {
                Navigator.push(
          context, MaterialPageRoute(builder: (context) => QuranListScreen()));
          },
        ),
        _buildFeatureButton(
          context,
